...
class Database {
  ...
  Future<void> _initConnection() async {
    _connection = PostgreSQLConnection(_ip, _port, _database,
        username: _username, password: _password, timeoutInSeconds: 3);
    try {
      await _connection.open();
    } on PostgreSQLException {
      throw DatabaseException(
          '(Database)_initConnection: Wrong DATABASE ($_database), USERNAME ($_username) or PASSWORD ($_password)');
    } on TimeoutException {
      throw DatabaseTimeoutException(
          '(Database)_initConnection: Database disconnected');
    } catch (e) {
      throw Exception('(Database)_initConnection: Connection failed\n$e');
    }
  }
  ...
  Future<void> testConnection() async {
    try {
      ...
      throw DatabaseTimeoutException(
          '(Database)_initConnection: Database disconnected or make too much time to answer');
    } catch (e) {
      throw Exception(e);
    }
  }
  ...
  Future<List<Cell>> _cellsFromRawValues(PostgreSQLResult results) async {
    try {
      ...
      throw DatabaseException('(Database)_cellsFromRawValues:\n$e}');
    }
  }
  ...
  Future<List<Cell>> selectCells(String matchWord) async {
    try {
      ...
      var results = await _connection.query(
          "SELECT * FROM cell WHERE title LIKE '%$matchWord%' ORDER BY title;");
      await _connection.close();
      ...
      throw DatabaseException('(Database)$e');
    }
  }
  ...
  Future<List<Sheet>> selectSheets(int idCell) async {
    try {
      ...
      var sheets = <Sheet>[];
      await _initConnection();
      sheetsRaw = await _connection.query(
          'SELECT id, title, subtitle, idorder FROM sheet WHERE idcell = $idCell ORDER BY idorder;');
      await _connection.close();
      ...
      throw DatabaseException('(Database)selectSheets:\n$e');
    } on TimeoutException {
      throw DatabaseTimeoutException(
          '(Database)_initConnection: Database disconnected');
    } catch (e) {
  ...
  Future<Sheet> selectSheet(int idCell, int sheetIndex) async {
    await _initConnection();
    var sheetRaw = await _connection.query('SELECT id, title, subtitle FROM '
        'sheet WHERE idcell = $idCell AND idorder = $sheetIndex;');
    await _connection.close();
    return Sheet(sheetRaw[0][0] as int, idCell, sheetRaw[0][1] as String,
        sheetRaw[0][2] as String, sheetIndex);
  }
  ...
  Future<List<Element>> selectElements(int idSheet) async {
    try {
      ...
      await _initConnection();
      checkboxes = await _connection.query(
          'SELECT id, text, is_checked, elem_order FROM checkbox WHERE id_sheet = $idSheet ORDER BY elem_order;');
      images = await _connection.query(
          'SELECT id, image_preview, elem_order FROM image WHERE id_sheet = $idSheet ORDER BY elem_order;');
      texts = await _connection.query(
          'SELECT id, text, type, elem_order FROM text WHERE id_sheet = $idSheet ORDER BY elem_order;');
      await _connection.close();
      ...
      throw DatabaseException('(Database)selectElements:\n$e');
    } catch (e) {
      throw Exception(e);
    }
  }
  ...
  Future<String> selectRawImage(int idImg) async {
    try {
      await _initConnection();
      var rawImgResult = await _connection
          .query('SELECT image_raw FROM image WHERE id = $idImg;');
  ...
  Future<void> addCell(String title, String subtitle, String type) async {
    try {
      ...
      await _connection.query(
          "CALL add_cell('$title'::text, '$subtitle'::text, $typeInt::integer);");
      await _connection.close();
    } on PostgreSQLException catch (e) {
      throw DatabaseException('(Database)addCell: Wrong entries\n$e');
    } catch (e) {
      throw DatabaseException('(Database)addCell: Connection lost\n$e');
    }
  }

  Future<void> addSheet(
      int idCell, String title, String subtitle, int idOrder) async {
    try {
      await _initConnection();
      await _connection.query(
      ...
      throw DatabaseException('(Database)addSheet: Wrong entries\n$e');
    } catch (e) {
      throw DatabaseException('(Database)addSheet: Connection lost\n$e');
    }
  }
  ...
  Future<void> addCheckbox(int idSheet) async {
    try {
      ...
      throw DatabaseException('(Database)addCheckbox: Connection lost\n$e');
    }
  }

  Future<void> addImage(
      Uint8List imgPreview, Uint8List? imgRaw, int idSheet) async {
    try {
      var jsonImgPreview = jsonEncode({'img_preview': imgPreview});
      var jsonImgRaw = jsonEncode({'img_raw': imgRaw});
      await _initConnection();
      await _connection.query(
          "CALL add_image($idSheet::bigint, '$jsonImgPreview'::text, '$jsonImgRaw'::text);");
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)addImage: Connection lost\n$e');
    }
  }
  ...
  Future<void> addText(int type, int idSheet) async {
    try {
      await _initConnection();
      await _connection
          .query('CALL add_text($idSheet::bigint, $type::integer);');
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)add_text: Connection lost\n$e');
    }
  }
  ...
  Future<void> deleteCell(int idCell) async {
    try {
      ...
      throw DatabaseException('(Database)deleteCell: Connection lost\n$e');
    }
  }
  ...
  Future<void> deleteSheet(int idSheet) async {
    try {
      ...
      throw DatabaseException('(Database)deleteSheet: Connection lost\n$e');
    }
  }
  ...
  Future<void> deleteElement(int idElement) async {
    try {
      ...
      throw DatabaseException('(Database)deleteElement:\n$e');
    }
  }
  ...
  void updateCell(int id, String title, String subtitle) async {
    try {
      await _initConnection();
      await _connection.query(
          "CALL update_cell($id::bigint, '$title'::text, '$subtitle'::text);");
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)updateCell: Connection lost\n$e');
    }
  }
  ...
  void updateSheet(int id, String title, String subtitle) async {
    try {
      await _initConnection();
      await _connection.query(
          "CALL update_sheet($id::bigint, '$title'::text, '$subtitle'::text);");
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)updateSheet: Connection lost\n$e');
    }
  }
  ...
  void updateCheckbox(int id, bool isCheck, String text) async {
    try {
      await _initConnection();
      await _connection.query(
          "CALL update_checkbox($id::bigint, $isCheck::boolean, '$text'::text);");
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)updateCheckbox: Connection lost\n$e');
    }
  }
  ...
  void updateTexts(int id, String text, int type) async {
    try {
      ...
      throw DatabaseException('(Database)updateTexts: Connection lost\n$e');
    }
  }
    ...
    try {
      ...
      for (var i = 0; i < sheets.length; i++) {
        if (sheets[i].idOrder != i) {
          await _connection.query(
              'CALL update_sheet_order(${sheets[i].id}::bigint, ${sheets[i].idOrder}::int);');
        }
      }
      await _connection.close();
    } catch (e) {
      throw DatabaseException('(Database)updateSheetOrder:\n$e');
    }
  }
  ...
  Future<void> updateElementOrder(List<Element> elements) async {
    try {
      ...
      throw DatabaseException('(Database)updateElementOrder:\n$e');
    }
  }

  Future<void> updateDatabaseElementOrder(
